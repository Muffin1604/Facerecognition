...
name = {0: "Karan", 1: "Rakesh", 2: "Kalyan"}  # Change names accordingly.
    ...
    for face in faces_detected:
        ...
        if predicted_name == "Unknown":
            if current_time - last_unknown_time >= timedelta(minutes=2):
                ...
                frame_file = f"frame-images/frame_{predicted_name}_{timestamp}.jpg"
            ...
            if predicted_name not in recognized_names:
                recognized_names[predicted_name] = current_time
                timestamp = current_time.strftime('%Y-%m-%d %H:%M:%S')
                ...
                if not os.path.isfile(csv_file):
                    ...
                    new_entry.to_csv(csv_file, mode='a', header=False, index=False)
            elif current_time - recognized_names[predicted_name] >= timedelta(hours=2):
                recognized_names[predicted_name] = current_time
                timestamp = current_time.strftime('%Y-%m-%d %H:%M:%S')
                frame_file = f"frame-images/recognised/frame_{predicted_name}_{timestamp}.jpg"
                cv2.imwrite(frame_file, test_img)
